...
@section('content')
<div class="mb-4">
    <h1 class="text-xl font-bold text-gray-800 dark:text-white">Tambah Data Baru</h1>
    <p class="text-sm text-gray-500 dark:text-gray-400">Tambah pelanggan, supplier, karyawan, atau sopir baru</p>
</div>

<div class="bg-white dark:bg-gray-800 rounded-lg shadow max-w-2xl">
    <form action="{{ route('parties.store') }}" method="POST" class="p-6">
        @csrf

        <div class="mb-4">
            <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Tipe <span class="text-red-500">*</span></label>
            <select name="type" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 @error('type') border-red-500 @enderror dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                <option value="">Pilih Tipe</option>
                <option value="customer" {{ old('type') == 'customer' ? 'selected' : '' }}>Pelanggan</option>
                <option value="supplier" {{ old('type') == 'supplier' ? 'selected' : '' }}>Supplier</option>
                <option value="employee" {{ old('type') == 'employee' ? 'selected' : '' }}>Karyawan</option>
                <option value="driver" {{ old('type') == 'driver' ? 'selected' : '' }}>Sopir</option>
            </select>
            @error('type')
        ...
        <div class="mb-4">
            ...
            <textarea name="address" rows="3" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('address') }}</textarea>
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Catatan</label>
            <textarea name="notes" rows="2" class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('notes') }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button type="submit" class="bg-primary-500 hover:bg-primary-600 text-white font-bold py-2 px-4 rounded-lg text-sm transition-all">Simpan</button>
        </div>
    </form>
</div>
@endsection